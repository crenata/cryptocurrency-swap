...
class Home extends PureComponent {
    constructor(props) {
        ...
        this.state = {
            from: "",
            to: "",
            switch: Config.SwapSwitch.FromTo
        };
    }

    setFloatValue(name, value) {
        let rate = parseFloat(this.context.rate.toString());
        if (IsEmpty(value)) {
            this.setState({
                from: "",
                to: ""
            ...
            if (floatRegex.test(value)) {
                if (name === "from") {
                    this.setState({
                        from: value,
                        to: parseFloat(value) * rate
                    });
                } else if (name === "to") {
                    this.setState({
                        from: parseFloat(value) / rate,
                        to: value
                    });
                } else {
    ...
    switch() {
        if (this.state.switch === Config.SwapSwitch.FromTo) {
            this.setState({
                switch: Config.SwapSwitch.ToFrom
            });
        } else {
            this.setState({
                switch: Config.SwapSwitch.FromTo
    ...
    buyTokens() {
        if (!IsEmpty(this.state.from)) {
            if (!IsEmpty(this.context.bnbSwap)) {
                this.context.setLoading(true, () => {
                    this.context.bnbSwap.deployed().then((data) => {
                        data.buyTokens({
                            from: this.context.account,
                            value: this.context.web3.utils.toWei(this.state.from.toString(), "ether")
                        }).then((result) => {
                        ...
                        this.setState({
                            ...
                            ErrorNotDeployed(this.context.bnbSwap, error);
                            this.context.setLoading(false);
                        });
                    }).finally(() => {});
                });
            } else {
                console.error("bnbSwap is null");
            }
        } else {
    ...
    sellTokens() {
        if (!IsEmpty(this.state.to)) {
            if (!IsEmpty(this.context.bep20Token)) {
                if (!IsEmpty(this.context.bnbSwap)) {
                    this.context.setLoading(true, () => {
                        this.context.bep20Token.deployed().then((bep20TokenData) => {
                            this.context.bnbSwap.deployed().then((data) => {
                                let amount = this.context.web3.utils.toWei(this.state.to.toString(), "ether");
                                bep20TokenData.approve(this.context.bnbSwap.address, amount, {
                                    from: this.context.account
                                }).then((resultApproval) => {
                                    setTimeout(() => {
                                        this.context.setLoading(true, () => {
                                            data.sellTokens(amount, {
                                                from: this.context.account
                                            }).then((result) => {
                                                console.log('result', result);
                                            }).catch((error) => {
                            ...
                            this.setState({
                                ...
                                ErrorNotDeployed(this.context.bep20Token, error);
                                this.context.setLoading(false);
                            });
                        }).finally(() => {});
                    });
                ...
                console.error("BEP20Token is null");
            }
        } else {
            console.error("Value is null");
        }
                ...
                {(web3Context) => {
                    ...
                    let from = (
                        <div className="from-swap">
                            <div className="d-flex justify-content-between align-items-center">
                                <label htmlFor="from-swap-input" className="small">{this.state.switch === Config.SwapSwitch.FromTo ? "From" : "To"}</label>
                            ...
                            <div className="input-group mt-2">
                                ...
                                <input
                                    ...
                                    onChange={(event) => this.setFloatValue(event.target.name, event.target.value)}
                                />
                            </div>
                        </div>
                    );
                    let to = (
                        <div className="to-swap">
                            <div className="d-flex justify-content-between align-items-center">
                                <label htmlFor="to-swap-input" className="small">{this.state.switch === Config.SwapSwitch.ToFrom ? "From" : "To"}</label>
                                        ...
                                        <div className="row">
                                            <div className="col-12 col-sm-12 col-md-8 col-lg-6 col-xl-6 offset-sm-0 offset-md-2 offset-lg-3 offset-xl-3">
                                                <div className="p-4 border-radius-1rem box-shadow-primary bgc-white-opacity-95">
                                                    {this.state.switch === Config.SwapSwitch.FromTo ? from : to}
                                                    <div className="switch-swap my-3">
                                                        <div className="d-flex justify-content-center">
                                                            <button
                                                                className="rounded-circle border d-flex align-items-center bgc-CBD3D5 opacity-75"
                                                                style={{
                                                                    width: 28,
                                                                    height: 28
                                                                }}
                                                    ...
                                                    {this.state.switch === Config.SwapSwitch.ToFrom ? from : to}
                                                    <button className="btn text-white w-100 mt-3 bgc-1C152D-opacity-95 border-radius-1rem" onClick={() => this.submit()}>
